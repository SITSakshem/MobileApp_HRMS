...
import '../../data/model/mobile_punch_report_response.dart';
...
class MobilePunchAttendanceReportScreen extends StatefulWidget {
  const MobilePunchAttendanceReportScreen({super.key});

  @override
  State<MobilePunchAttendanceReportScreen> createState() => _MobilePunchAttendanceReportState();
}

class _MobilePunchAttendanceReportState extends State<MobilePunchAttendanceReportScreen> {
  late UserData userData;
  List<MobilePunchReport> mobilePunchReport = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Mobile Punch Report'),
      ),
      body: Container(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              Visibility(visible: isSubmitPressed && !dataLoading && mobilePunchReport.isEmpty, child: const Center(child: Text('No data found'))),
              Visibility(
                  visible: isSubmitPressed && mobilePunchReport.isNotEmpty,
                  child: Container(
                    //padding: const EdgeInsets.all(8.0),
                    ...
                    child: rowDataMobilePunchReportHead(context, 'Name', 'Date', 'PunchType','PunchTime', 'Address'),
                  )),
              Expanded(
                  child: ListView.separated(
                      ...
                      itemCount: mobilePunchReport.length,
                      itemBuilder: (BuildContext context, int index) {
                        var currentData = mobilePunchReport[index];
                        return Container(
                            ...
                            //context, String empDetails, String date, String punchTypeName, String locationAddress
                            child: rowDataMobilePunchReport(context, currentData.empName, currentData.attendanceDt, currentData.punchTypeName, currentData.punchTime, currentData.locationAddress));
                      })),
            ],
          ),
        ),
  ...
  void checkAttendanceReport() async {
    setState(() {
      dataLoading = true;
      isSubmitPressed = true;
      mobilePunchReport.clear();
    });
    var result = await AttendanceRepository().getMobilePunchReport(userData, fromDate, toDate, downLineEmployeeId);
    if (result is Success) {
      mobilePunchReport = result.data as List<MobilePunchReport>;
    }
    setState(() {
      dataLoading = false;
    });
